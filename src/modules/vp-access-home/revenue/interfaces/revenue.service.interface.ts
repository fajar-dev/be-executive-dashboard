export interface IRevenueService {
    getSummary(branchId: string, year: number): Promise<{ period: string; branch: string; total: number }[]>
    getHomepaid(branchId: string, year: number): Promise<{ period: string; branch: string; total: number }[]>
